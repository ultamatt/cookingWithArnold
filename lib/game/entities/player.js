...
.defines(
    function(){
        EntityPlayer = ig.Entity.extend({
            ...
            animSheet: new ig.AnimationSheet('media/CHARACTERS/PlayerArnold2.png', 40, 50),
            sheetSwap: 0,
            size: {x:22, y:46},
            offset: {x:8, y:0},
            flip: false,
            ...
            setupAnimation: function(offset){
                ...
                this.addAnim('die', 0.5, [8+offset, 33, 34]);
                ...
                if(!this.hit || this.invincible){
                    ...
                    if(ig.input.state('shoot')){
                        switch(this.weapon){
							...
							case(2):
									//ANTIGRIDDLE
									ig.game.spawnEntity( EntityAntimatterStream, this.pos.x  + bulletSpread(), this.pos.y + bulletSpread(), {flip:this.flip});
									ig.game.spawnEntity( EntityAntimatterStream, this.pos.x  + bulletSpread(), this.pos.y + bulletSpread(), {flip:this.flip});
									ig.game.spawnEntity( EntityAntimatterStream, this.pos.x  + bulletSpread(), this.pos.y + bulletSpread(), {flip:this.flip});
									ig.game.spawnEntity( EntityAntimatterStream, this.pos.x  + bulletSpread(), this.pos.y + bulletSpread(), {flip:this.flip});
									ig.game.spawnEntity( EntityAntimatterStream, this.pos.x  + bulletSpread(), this.pos.y + bulletSpread(), {flip:this.flip});
							...
							case(3):
                                    //EMULSIFIER!
									if(ig.input.pressed('shoot')){
                                    	ig.game.spawnEntity( EntityEmulsifierStream, this.pos.x, this.pos.y, {flip:this.flip});
