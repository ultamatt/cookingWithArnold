    ...
    function(){
        EntityPlayer = ig.Entity.extend({
            ...
            update: function(){
                ...
                ig.input.state('left') == true ? this.input = 'left' : null; 
                ig.input.state('right') == true ? this.input = 'right' : null; 
                ig.input.state('up') == true ? this.input = 'up' : null; 
                ig.input.state('down') == true ? this.input = 'down' : null; 
            ...
            handle_movement_input: function(direction) {
                ...
                if (this.climbing && leftOrRight) {
                    console.log("Getting off ladders.");
                    this.jump_off_ladder(direction);
                } else if (leftOrRight && (this.standing || this.over_ladder)) {
                    console.log("Running while atop ladders and on the ground.");
                    this.set_velocity_by_direction(direction);
                } else if (leftOrRight && !this.standing) {
                    console.log("Prevent moving horizontally by " 
                        + "walking over a ledge while holding key.");
                    this.stop();
                } else if (this.climbing && 
                      ((direction === 'up' && this.at_top_of_ladder()) 
                    || (direction === 'down' && this.at_bottom_of_ladder()))){ 
                    console.log("Getting off at the top and the bottom of ladders.");
                    this.climbing = false;
                } else if (upOrDown && this.climbing) {
                    console.log("Moving up and down ladders.");
                    this.set_velocity_by_direction(direction);
                } else if (upOrDown && this.over_ladder) {
                    console.log("Getting on to ladders.");
                    this.mount_ladder();                    
                } else if (upOrDown && this.standing) {
                    console.log("Prevent moving horizontally by tapping "
            ...
            stop: function() {
                this.accel.x = 0; // never move horizontally without input
            ...
            jump_off_ladder: function(side) {
                console.log("trying to run jump_off_ladder");
                this.stop();
                if(!this.can_move_in_direction(side)) {
